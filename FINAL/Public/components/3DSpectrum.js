import ImprovedNoise from '../scripts/perlinNoise.js';
import config from '../scripts/config.js';

//let ImprovedNoise = window.config;
  ...
  tick: function (time) {
    ...
    for (let i = 0; i < children.length; i++) {
      children[i].setAttribute('scale', {
        x: 1,
        y: Math.min(data.max, Math.max(levels[i*2] * data.multiplier, 0.05)),
        z: 1
      });
      let color = new THREE.Color().setHSL(colors[i], 1, levels[i*2]/255);
